...
DomNode::DomNode(uint32_t id, uint32_t pid, int32_t index, std::string tag_name, std::string view_name,
                 ...
                 std::weak_ptr<RootNode> weak_root_node, void* layout_config)
    : id_(id),
      pid_(pid),
      index_(index),
  ...
  layout_node_ = hippy::dom::CreateLayoutNode(layout_config);
}

DomNode::DomNode(uint32_t id, uint32_t pid, std::weak_ptr<RootNode> weak_root_node, void* layout_config)
    : DomNode(id, pid, 0, "", "", nullptr, nullptr, std::move(weak_root_node), layout_config) {}

DomNode::DomNode() : DomNode(0, 0, {}, nullptr) {}
